...
expt4_model_order <- c("Human", "MIGHT", "trace", "Pursuit", "FUb", "FUb-fixed")
```
...
expt4_models <- read_csv("../model_code/results/might_expt4_results.csv") %>%
  rbind(read_csv("../model_code/results/trace_expt4_results.csv")) %>%
...
expt4_test_all <- expt4_aggr_test %>%
  ...
  ) %>% write_csv("../model_comparison/expt4.csv")  #%>%
  # filter(model_g %in% c("MIGHT", "Human")) %>%
  # mutate(
  #   model_g = factor(model_g, levels = c("MIGHT", "Human")),
  #   Condition = factor(case_when(
  #     condition == "same" ~ "Lexicon",
  #     condition == "switch" ~ "Learning-space"
  #   ), levels = c("Lexicon", "Learning-space"))
  # )

ggplot(filter(expt4_test_all, word_type == "target"), 
       aes(x=Condition, y=mean_acc, fill=block
                                    )) +
  geom_col_pattern(aes(pattern = participant), position="dodge", width=0.8) +
...
ggsave("expt4_test_diss_pres.png", width=8, height=4, units="in", dpi="print")

---
title: "Expt 1, Expt 2 Analysis"
author: "Christine Soh Yue"
date: "2025-01-13, updated 2026-09-17"
...
expt1_fubg_pred <- tibble(
  ...
  "sd_acc" = c(NA, NA, NA, NA, NA, NA,
               NA, NA, NA, NA, NA, NA), 
  "ci" = c(NA, NA, NA, NA, NA, NA,
           NA, NA, NA, NA, NA, NA), 
...
expt1_model_plot <- ggplot(data = filter(expt1_model_pred
  ...
  geom_errorbar(aes(x=condition, ymin=mean_acc+ci,
                    ymax=mean_acc-ci),
                width=.3,
...
## Results of Human Performance: Retrieval of Hypotheses

```{r expt1_learning_retrieval}
expt1_data %>%
  filter(
    phase == "learning"
  ) %>%
...
```{r expt1_learning_retrieval_exp2}
expt1_exp2_accuracy <- expt1_data %>%
  filter(
    phase == "learning", exposure == 2
  ) %>%
  group_by(condition, word_type) %>%
  summarise(
    count = n(),
    mean_accuracy = mean(typed_accuracy),
    sd_accuracy = sd(typed_accuracy),
...
expt1_first_hypotheses <- expt1_data %>%
  filter(
    phase == "learning",
...
expt1_learning_typed_from_hyp <- expt1_data %>%
  filter(
    phase == "learning"
  ) %>%
  mutate(
 ...
 expt1_learning_typed_from_hyp %>%
  ...
  filter(
   ...
   filter(exposure == 2) %>%
   left_join(filter(expt1_exp2_accuracy, word_type == "target")) %>%
   mutate(
     from_hyp_but_incorrect = mean_typed_from_hyp - mean_accuracy
   ) %>%
   select(
     exposure, condition, from_hyp_but_incorrect, mean_typed_from_hyp, mean_accuracy
   )
```

## Results of Human Performance: Performance at Test
...
expt1_confidence <- expt1_2_results %>%
  mutate(
    minb = mean_acc - ci,
    maxb = mean_acc + ci
  ) %>%
  ...
  select(!c(count, participants, sd_acc, ci)) %>%
  pivot_wider(names_from = expt, values_from = c(mean_acc, minb, maxb)) %>%
  mutate(
    min = `minb_Expt 1`,
    max = `maxb_Expt 1`
  ) %>%
  select(!c(`minb_FUb`, `maxb_FUb`, `minb_FUb-fixed`, `maxb_FUb-fixed`, 
            minb_Pursuit, maxb_Pursuit, minb_MIGHT, maxb_MIGHT, 
            # minb_TRACE, maxb_TRACE, 
            `minb_Expt 1`, `maxb_Expt 1`)) %>%
  mutate(
    FUb = mean_acc_FUb > min & mean_acc_FUb < max,
    FUb_f = `mean_acc_FUb-fixed` > min & `mean_acc_FUb-fixed` < max,
    MIGHT = mean_acc_MIGHT > min & mean_acc_MIGHT < max,
    Pursuit = mean_acc_Pursuit > min & mean_acc_Pursuit < max
  )

expt2_confidence <- expt1_2_results %>%
  mutate(
    minb = mean_acc - ci,
    maxb = mean_acc + ci
  ) %>%
...
expt1_2_paper_plot <- ggplot(filter(expt1_2_results, expt != "Trace"), aes(x=condition, y=mean_acc, fill=word_type_graph)) +
...
expt1_typed_learning <- expt1_data %>%
  filter(
    phase == "learning"
  )  %>%
  mutate(
